#!/usr/bin/env node
// BUY-30620 Stock — page fresh merchants (parallel to Hunt2, independent cursor)
...
const PRODUCTIVE_FEED = `${ROOT}/data/productive_feeds/stock.ndjson`;
const OUT_DIR = `${ROOT}/data/buy30620-stock`;
const CKPT = `${OUT_DIR}/checkpoint.json`;
const LOG = `${ROOT}/logs/buy30620_stock.log`;
const R2_TEARDOWN_LOG = `${ROOT}/logs/buy30620_stock_r2.log`;
const LANE = 'stock';
const LANE_SOURCE = 'shopify_buy30620_stock';
...
const PRODUCTIVE_ONLY = process.env.STOCK_PRODUCTIVE_ONLY !== 'false';
...
function throughputWindowThrottleMs() {
  ...
  //   BUY64173_DISABLE_THROTTLE=1 node scripts/buy30620-stock-page-lane.mjs
...
async function pageMerchant(domain) {
  ...
  return products.map(p => toProductRecord(domain, LANE_SOURCE, p, { lane: 'stock', currency }));
}
...
async function runCycle(domains, ckpt) {
  ...
  log(`stock cycle ${ckpt.cycle}: ${batch.length} domains → ${hits} hit → ${total} products → ${outFile}`);
  ckpt.ingested += total;
  saveCkpt(ckpt);
  return outFile;
}
...
function fireR2Teardown(outFile) {
  ...
  } }).catch((e) => log(`stock r2_teardown error: ${e.message || e}`));
}

async function main() {
  let domains = loadFeed();
  log(`stock loaded ${domains.length} fresh merchants`);
  const ckpt = loadCkpt();
  log(`stock starting cursor=${ckpt.cursor} cycle=${ckpt.cycle}`);
  let consecutiveEmpty = 0;
  while (true) {
    const throttleMs = throughputWindowThrottleMs();
    if (throttleMs > 0) {
      log(`BUY-64173 throttle: pausing stock Shopify refresh for ${Math.ceil(throttleMs / 1000)}s during throughput window rollback=BUY64173_DISABLE_THROTTLE=1`);
    ...
    catch (e) { log(`stock cycle error: ${e.message}`); }
    ...
    if (produced === 0) {
      ...
      if (consecutiveEmpty >= DEAD_ZONE_THRESHOLD && domains.length > 0) {
        ...
        log(`stock DEAD-ZONE SKIP: ${consecutiveEmpty} empty cycles${isInTrancoZone ? ' (TRANCO ZONE)' : ''} -> cursor -> ${ckpt.cursor}`);
        saveCkpt(ckpt);
        consecutiveEmpty = 0;
      }
    } else {
